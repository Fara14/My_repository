...
# (19) проверяет, является ли строка корректной записью IP-адреса. Если да, вернуть True, иначе False
def f19(ip):
    a = ip.split('.')
    if len(a) != 4:
        return False
...
# (20) извлекает из строки все символы, являющиеся цифрами и возвращает эту новую строку
def f20(string):
    digits = ""
    for digit in string:
...
# (21) принимает множество чисел на вход (*argc) и выводит среднее арифметическое для нечетных чисел
# и отдельно среднее арифметическое для четных
def f21(*args):
    chetnye = []
    nechetnye = []
    for el in args:
...
# (22) возвращает индексы ненулевых элементов списка
def f22(ls):
    result = []
...
# (23) возвращает ближайший элемент к заданному значению списка
def f23(ls, n):
...
# (24) после каждой гласной добавляет букву "с" и повторяет эту гласную (аса, еса, иси, и т.д.)
def f24(st):
...
# (25) возвращает три последовательных элемента в списке, сумма которых максимальна
def f25(ls):
...
# (26) проверяет есть ли в массиве повторяющиеся элементы
def f26(ls):
    a = list(set(ls))
    if len(ls) == len(a):
        return False
...
print(f"(26)проверяет есть ли в массиве повторяющиеся элементы\n{f26([1, 2, 0, 0, 4, 0])}\n")


# (27) возвращает количество элементов списка, которые отличны от наибольшего элемента не более чем на 10%
def f27(ls):
    result = []
    max_el = max(ls)
    pr = max_el * 0.1
...
print(f"(27) возвращает количество элементов списка, которые отличны от наибольшего элемента не более чем на 10%\n"
      f"{f27([3, 4, 6, 10, 11, 13, 12])}\n")


# (28) на вход получает список C, и меняет знак у элементов, значения которых между A и B
def f28(c, a, b):
...
print(f"(28) на вход получает список C, и меняет знак у элементов, значения которых между A и B\n"
      f"{f28([1,5,-7,13,-1], 2, 11)}\n")


# (29) проверяет одинаковы ли 2 списка
def f29(ls1, ls2):
    return ls1 == ls2
...
# (30) "сжимает" список, то есть перемещает все ненулевые элементы в левую часть списка, не меняя их порядок,
...
print(f"(30) 'сжимает' список, то есть перемещает все ненулевые элементы в левую часть списка, не меняя их порядок,\n"
      f"а все нули - в правую часть\n {f30([4, 0, 5, 0, 3, 0, 0, 5])}\n")


# (31) выводит все элементы списка с четными индексами
def f31(ls):
    return ls[::2]
...
# Не понял задачи:
# (32) находит индексы локальных максимумов списка
# (33) находит наименьшее расстояние между локальными максимумами списка
